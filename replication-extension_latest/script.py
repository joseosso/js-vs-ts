...
class SonarQubeDance:
    ...
    def executeAnalysis(self, token):
        ...
        os.system(
            f'sonar-scanner -D"sonar.projectKey={self.repoFullName}" -D"sonar.host.url=http://localhost:9000" -D"sonar.login={token}"'
        )
...
#sampling.checkRepoByCharacteristics(1,100)
...
#sonarQubeDance.createAndAnalyzeRepos(1,101)

#packageJson = PackageJson()
#eslint = ESLint()
#eslint.createEslintReport(1,101)

#commaSeparatedValues = CSV("Metrics.csv")
#sampling.writeIssueDataFromJsonToCSV()
#sampling.writeCommitsDataFromJsonToCSV()
#packageJson = PackageJson()
#framework = Framework()
#framework.getFramework(1,101)
...
metrics = Metrics()
#metrics.calculateMetricPerLoc("cognitive_complexity")

test = Test()
#alternative (string): alternative = less, if tsvals less than jsval. alternative = greater, if tsvals greater than jsval. Else alternative = two-sided
#metric (string): Metrics can be: avg_bug-issue_time, code-smells_ncloc, cognitive-complexity_ncloc, bug-fix-commits_ratio
test.testHypothesis(metrics.getMetricWithoutInf('avg_bug-issue_time'),'less')

#printDiagramms = PrintDiagramms()
#printDiagramms.printScatterplot("code-smells_ncloc", [0, 10000], [0, 10000])
